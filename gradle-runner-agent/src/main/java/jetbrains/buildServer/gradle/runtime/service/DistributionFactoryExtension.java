...
/**
 * Sets up gradle wrapper distribution for {@link org.gradle.tooling.GradleConnector}
 */
public final class DistributionFactoryExtension extends DistributionFactory {

    private DistributionFactoryExtension() {
        super(Time.clock());
    }
    ...
    public static void setWrappedDistribution(GradleConnector connector, String wrapperPropertyFile) {
        ...
        if (propertiesFile.exists()) {
            ...
            if (wrapper.getDistribution() != null) {
                Distribution distribution = new DistributionFactoryExtension().getWrappedDistribution(propertiesFile);
                try {
    ...
    private Distribution getWrappedDistribution(File propertiesFile) {
        ...
        return getDownloadedDistribution(DefaultGradleVersion.current().getVersion());
    }

    private Distribution getDownloadedDistribution(String gradleVersion) {
        URI distUri = new DistributionLocator().getDistributionFor(DefaultGradleVersion.version(gradleVersion));
    ...
    private static final class ZippedDistribution implements Distribution {
        ...
        private static File determineRootDir(ConnectionParameters connectionParameters) {
            return (new BuildLayoutFactory()).getLayoutFor(
                    ...
                    connectionParameters.isSearchUpwards() != null
                            ? connectionParameters.isSearchUpwards()
                            : true)
                    .getRootDirectory();
        }
