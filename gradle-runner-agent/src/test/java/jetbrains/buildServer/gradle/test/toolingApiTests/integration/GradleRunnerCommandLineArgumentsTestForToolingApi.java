...
public class GradleRunnerCommandLineArgumentsTestForToolingApi extends GradleRunnerServiceMessageTestForToolingApi {
  ...
  public void should_NotFail_When_SupportedLongArgumentsArePassed(final String gradleVersion) throws Exception {
    ...
    List<String> supported = new ArrayList<>(GradleToolingCommandLineOptionsProvider
                                               ...
                                               .filter(arg -> VersionComparatorUtil.compare(getGradleVersionFromPath(gradleVersion), "9") < 0 || !arg.equals("--settings-file"))
                                               // The settings file option is not supported in Gradle 9+
  ...
  public void should_NotFail_When_SupportedShortArgumentsArePassed(final String gradleVersion) throws Exception {
    ...
    List<String> supported = new ArrayList<>(GradleToolingCommandLineOptionsProvider
                                               ...
                                               // The build file option is not supported in Gradle 9+
                                               .filter(arg -> VersionComparatorUtil.compare(getGradleVersionFromPath(gradleVersion), "9") < 0 || !arg.equals("-c"))
                                               // The settings file option is not supported in Gradle 9+
                                               .collect(Collectors.toList()));
